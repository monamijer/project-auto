...
foreach ($mensuelData as $r) $moisTotaux[$r['mois']] = $r['total'];

$parFormation = $pdo->query("SELECT * FROM v_rapport_formation")->fetchAll();
$perso = $pdo->prepare("SELECT COUNT(DISTINCT p.utilisateur_id) AS nb_eleves, COUNT(p.id) AS nb_paiements, COALESCE(SUM(p.montant),0) AS recettes FROM paiement p WHERE p.date_paiement BETWEEN ? AND ?" . ($formation ? " AND EXISTS(SELECT 1 FROM utilisateurs u WHERE u.id=p.utilisateur_id AND u.formation_id=?)" : ""));
$params = [$date_debut, $date_fin];
if ($formation) $params[] = $formation;
...
<div class="tab-pane fade show active" id="t1">
    <div class="card border-0 shadow-sm mb-4"><div class="card-header bg-white d-flex justify-content-between py-3"><h5 class="mb-0">Rapport mensuel</h5><form class="d-flex gap-2"><select name="annee" class="form-select form-select-sm" onchange="this.form.submit()"><?php foreach($annees as $a): ?><option value="<?= $a ?>" <?= $a==$annee?'selected':'' ?>><?= $a ?></option><?php endforeach; ?></select></form></div><div class="card-body"><canvas id="chartMensuel" height="100"></canvas></div></div>
    ...
    <?php foreach($moisLabels as $i => $label): $m=$i+1; ?><tr><td class="ps-3"><?= $label ?></td><td><strong><?= number_format($moisTotaux[$m],2) ?> $</strong></td></tr><?php endforeach; ?>
...
<div class="tab-pane fade" id="t2">
    <div class="card border-0 shadow-sm"><div class="card-header bg-white py-3"><h5 class="mb-0">Rapport personnalisé</h5></div><div class="card-body">
        <form method="GET" class="row g-3 mb-4"><input type="hidden" name="annee" value="<?= $annee ?>">
            ...
            <div class="col-md-4"><label class="form-label small">Formation</label><select name="formation" class="form-select"><option value="0">Toutes</option><?php foreach($formations as $f): ?><option value="<?= $f['id'] ?>" <?= $f['id']==$formation?'selected':'' ?>><?= htmlspecialchars($f['nom']) ?></option><?php endforeach; ?></select></div>
            <div class="col-md-2 d-flex align-items-end"><button type="submit" class="btn btn-primary w-100">Filtrer</button></div>
        </form>
        <div class="row g-3"><?php foreach([['Élèves','nb_eleves','people-fill','primary'],['Paiements','nb_paiements','cash','success'],['Recettes','recettes','graph-up','info'],['Leçons','lecons_ok','check-circle','warning']] as [$label,$key,$icon,$color]): $val=$key==='recettes'?number_format($persoData[$key]??0,2).' $':($persoData[$key]??0); ?><div class="col-md-3"><div class="card border-0 bg-<?= $color ?> bg-opacity-10 text-center p-3"><i class="bi bi-<?= $icon ?> text-<?= $color ?> fs-2 mb-2"></i><h3 class="fw-bold mb-0"><?= $val ?></h3><small class="text-muted"><?= $label ?></small></div></div><?php endforeach; ?></div>
    </div></div>
</div>
...
<div class="tab-pane fade" id="t3">
    ...
    <?php foreach($parFormation as $r): ?><tr><td class="ps-3 fw-medium"><?= htmlspecialchars($r['formation']) ?></td><td><?= $r['nb_inscrits'] ?></td><td><?= number_format($r['recettes'],2) ?> $</td><td><?= number_format($r['potentiel'],2) ?> $</td></tr><?php endforeach; ?>
    </tbody></table></div></div>
</div>

<div class="tab-pane fade" id="t4">
    <div class="card border-0 shadow-sm"><div class="card-header bg-white py-3"><h5 class="mb-0">Prévisions</h5><small class="text-muted">Basées sur 6 derniers mois (moy. <?= number_format($moyMens,2) ?> $/mois)</small></div><div class="card-body">
        <div class="row g-3 mb-4"><div class="col-md-4"><div class="card bg-success bg-opacity-10 text-center p-4"><h6>3 mois</h6><h2 class="text-success"><?= number_format($prevision3m,2) ?> $</h2></div></div><div class="col-md-4"><div class="card bg-primary bg-opacity-10 text-center p-4"><h6>6 mois</h6><h2 class="text-primary"><?= number_format($prevision6m,2) ?> $</h2></div></div><div class="col-md-4"><div class="card bg-warning bg-opacity-10 text-center p-4"><h6>12 mois</h6><h2 class="text-warning"><?= number_format($prevision12m,2) ?> $</h2></div></div></div>
...
new Chart(document.getElementById('chartCompar'),{type:'bar',data:{labels:<?= json_encode(array_column($parFormation,'formation')) ?>,datasets:[{label:'Recettes',data:<?= json_encode(array_column($parFormation,'recettes')) ?>,backgroundColor:'#22c55e',borderRadius:4},{label:'Potentiel',data:<?= json_encode(array_column($parFormation,'potentiel')) ?>,backgroundColor:'rgba(239,68,68,.3)',borderRadius:4}]},options:{responsive:true,scales:{y:{beginAtZero:true}}}});
