<?php
/**
 * pages/instructors.php — Moniteurs avec tri et archivage
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    ...
    $msg = callProcedure('CALL sp_ajouter_moniteur(?,?,?,?,?,@msg)', [trim($_POST['nom']), trim($_POST['prenom']), trim($_POST['nationalite']), trim($_POST['telephone']), (int) $_POST['experience']]);
...
if (isset($_GET['archive']) && hasPermission('crud_moniteurs')) {
    $sid = (int) $_GET['archive'];
    $pdo->prepare('UPDATE instructeurs SET deleted_at = NOW(), deleted_by = ? WHERE id = ?')->execute([$_SESSION['username'], $sid]);
    $pdo->prepare(
        "INSERT INTO archives (type_archive, nom_fichier, nb_enregistrements, periode_fin, cree_par) VALUES ('moniteurs', (SELECT CONCAT(prenom, ' ', nom) FROM instructeurs WHERE id = ?), 1, CURDATE(), ?)"
    )->execute([$sid, $_SESSION['username']]);
    $message = 'Moniteur archivé !';
}
...
if ($search !== '') {
    $instructors = array_filter($instructors, function ($i) use ($search) {
        return stripos($i['nom_complet'], $search) !== false || stripos($i['nationalite'] ?? '', $search) !== false || stripos($i['telephone'] ?? '', $search) !== false;
    });
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    <div><h1 class="h3 mb-1"><i class="bi bi-person-badge-fill me-2 text-primary"></i>Moniteurs</h1><p class="text-muted mb-0"><?= $total ?> moniteur(s)</p></div>
    <?php if (
        hasPermission('crud_moniteurs')
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3"><h5 class="mb-0"><i class="bi bi-list-ul me-2"></i>Liste des moniteurs</h5><span class="badge bg-primary rounded-pill"><?= $total ?></span></div>
    <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        <thead class="table-light"><tr>
            ...
            <th>Téléphone</th>
            <th class="sortable">Expérience</th>
            <th class="text-end pe-3">Actions</th>
        </tr></thead>
        <tbody>
        <?php if (empty($instructorsPage)): ?><tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-inbox display-4 d-block mb-2"></i>Aucun moniteur trouvé</td></tr>
        <?php else:foreach ($instructorsPage as $row): ?>
        <tr>
            ...
            <td class="text-end pe-3"><div class="btn-group btn-group-sm">
                ...
                <a href="?archive=<?= $row['id'] ?>" class="btn btn-outline-secondary" onclick="return confirm('Archiver ce moniteur ?')" title="Archiver"><i class="bi bi-archive"></i></a>
        ...
        <?php if (hasPermission('crud_moniteurs')): ?>
        <div class="modal fade" id="editModal-<?= $row['id'] ?>" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6"><label class="form-label">Nationalité</label><input type="text" name="nationalite" class="form-control" value="<?= htmlspecialchars(
                        $row['nationalite'] ?? ''
                    ) ?>"></div>
                    <div class="col-md-6"><label class="form-label">Téléphone</label><input type="text" name="telephone" class="form-control" value="<?= htmlspecialchars(
                        $row['telephone'] ?? ''
                    ) ?>"></div>
                    <div class="col-md-6"><label class="form-label">Expérience (ans)</label><input type="number" name="experience" class="form-control" value="<?= (int) $row[
                        'experience'
                    ] ?>" min="0" required></div>
                </div>
            </div>
