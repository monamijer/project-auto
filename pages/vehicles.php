<?php
/**
 * pages/vehicles.php — Véhicules avec tri et archivage
...
if (isset($_GET['archive']) && hasPermission('crud_vehicules')) {
    $sid = (int) $_GET['archive'];
    $pdo->prepare('UPDATE vehicules SET deleted_at = NOW(), deleted_by = ? WHERE id = ?')->execute([$_SESSION['username'], $sid]);
    $pdo->prepare(
        "INSERT INTO archives (type_archive, nom_fichier, nb_enregistrements, periode_fin, cree_par) VALUES ('vehicules', (SELECT CONCAT(marque, ' ', modele) FROM vehicules WHERE id = ?), 1, CURDATE(), ?)"
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    <div><h1 class="h3 mb-1"><i class="bi bi-car-front-fill me-2 text-primary"></i>Véhicules</h1><p class="text-muted mb-0"><?= $total ?> véhicule(s)</p></div>
    <?php if (
        hasPermission('crud_vehicules')
    ): ?><button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-lg me-1"></i>Ajouter</button><?php endif; ?>
</div>
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3"><h5 class="mb-0"><i class="bi bi-list-ul me-2"></i>Liste des véhicules</h5><span class="badge bg-primary rounded-pill"><?= $total ?></span></div>
    <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        <thead class="table-light"><tr>
            ...
            <th class="sortable">Immatriculation</th>
            <th class="sortable">Disponibilité</th>
            <th class="text-end pe-3">Actions</th>
        </tr></thead>
        <tbody>
        <?php if (empty($vehiclesPage)): ?><tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-inbox display-4 d-block mb-2"></i>Aucun véhicule trouvé</td></tr>
        <?php else:foreach ($vehiclesPage as $row): ?>
        <tr>
            <td class="ps-3"><span class="badge bg-secondary bg-opacity-10 text-secondary">#<?= $row['id'] ?></span></td>
            <td><div class="d-flex align-items-center"><div class="avatar-sm bg-secondary bg-opacity-10 rounded-circle me-2 d-flex align-items-center justify-content-center" style="width:36px;height:36px;"><i class="bi bi-car-front text-secondary"></i></div><span class="fw-medium"><?= htmlspecialchars(
            ...
            <td><?php if (
                $row['disponibilite']
            ): ?><span class="badge bg-success bg-opacity-10 text-success"><i class="bi bi-check2 me-1"></i>Disponible</span><?php else: ?><span class="badge bg-danger bg-opacity-10 text-danger"><i class="bi bi-x-lg me-1"></i>Indisponible</span><?php endif; ?></td>
            <td class="text-end pe-3"><div class="btn-group btn-group-sm">
                ...
                <a href="?archive=<?= $row['id'] ?>" class="btn btn-outline-secondary" onclick="return confirm('Archiver ce véhicule ?')" title="Archiver"><i class="bi bi-archive"></i></a>
