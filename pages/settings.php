...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_compte') {
    ...
    $msg = callProcedure('CALL sp_ajouter_compte(?,?,?,?,?,?,@msg)', [trim($_POST['utilisateur']), $hash, $_POST['role'], $_POST['date_expiration'], $_POST['statut'], trim($_POST['commentaire'] ?? '')]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'changer_mdp') {
    requirePermission('gestion_comptes');
    if ($_POST['nouveau_mdp'] !== $_POST['confirmer_mdp']) { $error = 'Les mots de passe ne correspondent pas.'; }
    else { $hash = password_hash($_POST['nouveau_mdp'], PASSWORD_BCRYPT); $msg = callProcedure('CALL sp_changer_mot_de_passe(?,?,@msg)', [(int)$_POST['id'], $hash]); $msg === 'OK' ? ($message = 'Mot de passe changé !') : ($error = $msg); }
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'bloquer') {
    requirePermission('gestion_comptes');
    if ((int)$_POST['id'] === (int)$_SESSION['user_id']) { $error = 'Vous ne pouvez pas vous bloquer.'; }
    else { $msg = callProcedure('CALL sp_bloquer_utilisateur(?,?,@msg)', [(int)$_POST['id'], trim($_POST['raison'] ?: 'Comportement inapproprié')]); $msg === 'OK' ? ($message = 'Utilisateur bloqué.') : ($error = $msg); }
}
if (isset($_GET['deverrouiller'])) { requirePermission('gestion_comptes'); callProcedure('CALL sp_deverrouiller_compte(?,@msg)', [(int)$_GET['deverrouiller']]); $message = 'Compte déverrouillé.'; }
if (isset($_GET['debloquer'])) { requirePermission('gestion_comptes'); callProcedure('CALL sp_debloquer_utilisateur(?,@msg)', [(int)$_GET['debloquer']]); $message = 'Utilisateur débloqué.'; }
if (isset($_GET['delete_compte'])) {
    requirePermission('gestion_comptes');
    if ((int)$_GET['delete_compte'] === (int)$_SESSION['user_id']) { $error = 'Vous ne pouvez pas vous supprimer.'; }
    else { callProcedure('CALL sp_supprimer_compte(?,@msg)', [(int)$_GET['delete_compte']]); $message = 'Compte supprimé.'; }
}
...
<div class="card shadow-sm border-0 mb-4">
    ...
    <div class="card-body">
        <form method="POST" class="row g-3">
            ...
            <?php $champs = ['nom_ecole'=>'Nom','telephone'=>'Téléphone','email_ecole'=>'Email école','adresse'=>'Adresse','devise'=>'Devise','smtp_user'=>'Email SMTP (Gmail)','smtp_pass'=>'Mot de passe app Gmail'];
            foreach ($champs as $k => $l): ?>
            <div class="col-md-6"><label class="form-label"><?= $l ?></label><input type="<?= $k==='smtp_pass'?'password':'text' ?>" name="config[<?= $k ?>]" class="form-control" value="<?= htmlspecialchars(getConfig($k)) ?>"></div>
            <?php endforeach; ?>
...
<div class="card shadow-sm border-0 mb-4">
    ...
    <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        ...
        <?php foreach ($comptes as $c):
            $badgeColor = match ($c['statut_reel']) {'actif'=>'bg-success bg-opacity-10 text-success','expire_bientot'=>'bg-warning bg-opacity-10 text-warning','expiré'=>'bg-danger bg-opacity-10 text-danger','suspendu'=>'bg-dark bg-opacity-10 text-dark',default=>'bg-secondary bg-opacity-10 text-secondary'};
            $badgeLabel = match ($c['statut_reel']) {'expire_bientot'=>'Expire bientôt','suspendu'=>'Bloqué',default=>ucfirst($c['statut_reel'])};
        ...
        <tr class="<?= $estVerrouille ? 'table-warning' : '' ?>">
            ...
            <td class="text-end pe-3"><div class="btn-group btn-group-sm">
                ...
                <?php if (in_array($c['statut_reel'], ['expiré','expire_bientot'])): ?><button class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#renewModal-<?= $c['id'] ?>" title="Renouveler"><i class="bi bi-arrow-clockwise"></i></button><?php endif; ?>
                ...
                <?php if ($c['id'] != $_SESSION['user_id']): ?>
                    ...
                    <?php else: ?><button class="btn btn-outline-dark" data-bs-toggle="modal" data-bs-target="#blockModal-<?= $c['id'] ?>" title="Bloquer"><i class="bi bi-lock"></i></button><?php endif; ?>
...
<div class="card shadow-sm border-0 mb-4">
    ...
    <?php foreach ($journal as $j): ?><tr><td class="ps-3"><?= htmlspecialchars($j['utilisateur']) ?></td><td><small><?= htmlspecialchars($j['heure_connexion']) ?></small></td><td><span class="badge <?= $j['statut']==='AUTORISÉE'?'bg-success bg-opacity-10 text-success':'bg-danger bg-opacity-10 text-danger' ?>"><?= htmlspecialchars($j['statut']) ?></span></td><td><small><?= htmlspecialchars($j['message'] ?? '') ?></small></td></tr><?php endforeach; ?>
    </tbody></table></div></div>
</div>
...
<div class="card shadow-sm border-0 mb-4"><div class="card-header bg-white py-3"><h5 class="mb-0"><i class="bi bi-clock-history me-2"></i>Journal activités</h5></div><div class="card-body p-0"><div class="table-responsive" style="max-height:350px;overflow-y:auto;"><table class="table table-sm mb-0"><thead class="table-light"><tr><th class="ps-3">Date</th><th>Utilisateur</th><th>Action</th><th>Module</th><th>Détails</th></tr></thead><tbody>
    <?php foreach ($activites as $a): $actBadge = match ($a['action']) {'AJOUT'=>'bg-success bg-opacity-10 text-success','MODIFICATION'=>'bg-warning bg-opacity-10 text-warning','SUPPRESSION'=>'bg-danger bg-opacity-10 text-danger',default=>'bg-secondary bg-opacity-10 text-secondary'}; ?>
    <tr><td class="ps-3"><small><?= htmlspecialchars($a['date_action']) ?></small></td><td><?= htmlspecialchars($a['utilisateur']) ?></td><td><span class="badge <?= $actBadge ?>"><?= htmlspecialchars($a['action']) ?></span></td><td><?= htmlspecialchars($a['module']) ?> <?= $a['element_id']?'#'.$a['element_id']:'' ?></td><td><small class="text-muted"><?= htmlspecialchars($a['details'] ?? '') ?></small></td></tr>
    <?php endforeach; ?>
...
<div class="card shadow-sm border-0"><div class="card-header bg-white py-3"><h5 class="mb-0"><i class="bi bi-info-circle me-2"></i>Infos système</h5></div><div class="card-body">
    <div class="row g-3 mb-3"><div class="col-md-4"><div class="p-3 bg-light rounded-3 text-center"><small>Élèves</small><strong class="fs-5"><?= $sysInfo['nb_eleves'] ?></strong></div></div><div class="col-md-4"><div class="p-3 bg-light rounded-3 text-center"><small>Moniteurs</small><strong class="fs-5"><?= $sysInfo['nb_moniteurs'] ?></strong></div></div><div class="col-md-4"><div class="p-3 bg-light rounded-3 text-center"><small>Véhicules</small><strong class="fs-5"><?= $sysInfo['nb_vehicules'] ?></strong></div></div></div>
    <div class="d-flex justify-content-between align-items-center"><small class="text-muted">PHP <?= phpversion() ?> · <?= date('Y-m-d H:i:s') ?></small><a href="<?= BASE_URL ?>/pages/corbeille.php" class="btn btn-outline-secondary btn-sm">Corbeille</a></div>
</div></div>
...
<div class="modal fade" id="addCompteModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="mb-3"><label class="form-label">Rôle</label><select name="role" class="form-select"><?php foreach ($roles as $role): ?><option value="<?= $role ?>" <?= $role==='stagiaire'?'selected':'' ?>><?= $rolesLabels[$role] ?></option><?php endforeach; ?></select></div>
