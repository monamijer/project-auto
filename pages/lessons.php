...
if ($search !== '') {
    $lessons = array_values(
        array_filter($lessons, fn($l) => stripos($l['student_nom'], $search) !== false || stripos($l['instructor_nom'], $search) !== false || stripos($l['vehicle_nom'], $search) !== false)
    );
}
if ($filter !== '') {
    $lessons = array_values(array_filter($lessons, fn($l) => $l['statut'] === $filter));
}
$total = count($lessons);
$totalPages = ceil($total / $perPage);
$offset = ($page - 1) * $perPage;
$lessonsPage = array_slice($lessons, $offset, $perPage);
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    ...
    ): ?><button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-calendar-plus me-1"></i>Planifier</button><?php endif; ?>
</div>
...
<div class="card shadow-sm border-0">
    ...
    $search
) ?>&filter=<?= urlencode($filter) ?>"><?= $i ?></a></li><?php endfor; ?>
