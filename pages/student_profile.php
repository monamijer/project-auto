...
$statut = 'Actif'; $statutBadge = 'success';
if ($solde <= 0 && $leconEffectuees >= 3) { $statut = 'Éligible examen'; $statutBadge = 'primary'; }
elseif ($solde > $student['formation_prix'] * 0.5) { $statut = 'Paiement en retard'; $statutBadge = 'danger'; }
...
<div class="row g-3">
    <div class="col-lg-4">
        <div class="card shadow-sm border-0 mb-3"><div class="card-body">
            ...
            <div class="text-center mb-3"><div class="rounded-circle bg-primary bg-opacity-10 d-inline-flex align-items-center justify-content-center mb-2" style="width:64px;height:64px;"><span class="text-primary fw-bold fs-5"><?= strtoupper(substr($student['prenom'], 0, 1) . substr($student['nom'], 0, 1)) ?></span></div></div>
        ...
        <div class="card shadow-sm border-0"><div class="card-body">
            ...
            <div class="row g-2 mt-2"><div class="col-6"><div class="p-2 bg-light rounded-3"><small class="text-muted d-block">Durée</small><strong><?= $student['formation_duree_mois'] ?> mois</strong></div></div><div class="col-6"><div class="p-2 bg-light rounded-3"><small class="text-muted d-block">Prix</small><strong><?= number_format($student['formation_prix'], 2) ?> $</strong></div></div></div>
        </div></div>
    </div>
    <div class="col-lg-8">
        <div class="card shadow-sm border-0 mb-3"><div class="card-header bg-white py-3"><h5 class="mb-0 small fw-bold text-muted">PAIEMENTS</h5></div><div class="card-body p-0">
            ...
            <?php foreach ($payments as $p): ?><tr><td class="ps-3"><?= date('d/m/Y', strtotime($p['date_paiement'])) ?></td><td><?= number_format($p['montant'], 2) ?> $</td><td><?= htmlspecialchars($p['methode']) ?></td></tr><?php endforeach; ?>
        ...
        <div class="card shadow-sm border-0"><div class="card-header bg-white py-3"><h5 class="mb-0 small fw-bold text-muted">LEÇONS</h5></div><div class="card-body p-0">
            ...
            <?php foreach ($lessons as $l): $badge = match ($l['statut']) { 'effectuée' => 'bg-success bg-opacity-10 text-success', 'annulée' => 'bg-danger bg-opacity-10 text-danger', default => 'bg-warning bg-opacity-10 text-warning' }; ?>
...
<div class="card shadow-sm border-0 mt-4" id="commentsSection">
    ...
    <div class="card-body">
        ...
        <div id="commentsList">
        <?php foreach ($comments as $cm): if (($cm['prive'] ?? 0) && !isAdmin()) continue; ?>
