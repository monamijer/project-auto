 ...
 * SELECT → v_archives, v_stats_archivage (Views SQL)
 * CRUD → sp_archiver_*, sp_desarchiver_document, sp_restaurer_archive
 */
session_start();
...
if (isset($_POST['action']) && $_POST['action'] === 'archiver') {
    ...
    if (in_array($type, ['eleves', 'paiements', 'lecons', 'documents'])) {
...
if (isset($_GET['restaurer'])) {
    $archiveId = (int) $_GET['restaurer'];
    $type = $pdo->query("SELECT type_archive FROM archives WHERE id = $archiveId")->fetchColumn();

    if ($type === 'documents') {
        $msg = callProcedure('CALL sp_desarchiver_document(?, @msg)', [$archiveId]);
    } else {
        $msg = callProcedure('CALL sp_restaurer_archive(?, @msg)', [$archiveId]);
    }

    $result = $pdo->query('SELECT @msg AS msg')->fetch();
    $message = $result['msg'] ?? 'Archive restaurée.';
    logActivity('RESTAURATION', 'archives', $archiveId);
}

// Vues
$archives = $pdo->query('SELECT * FROM v_archives')->fetchAll();
$statsArchive = $pdo->query('SELECT * FROM v_stats_archivage')->fetch();
...
<?php if ($message): ?><div class="alert alert-success alert-dismissible fade show"><?= htmlspecialchars(
    $message
) ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div><?php endif; ?>
<?php if ($error): ?><div class="alert alert-danger alert-dismissible fade show"><?= htmlspecialchars(
    $error
) ?><button type="button" class="btn-close" data-bs-dismiss="alert"></button></div><?php endif; ?>
...
<div class="row g-3 mb-4">
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100">
            <div class="card-body text-center">
                ...
                <h3 class="fw-bold"><?= $statsArchive['eleves_a_archiver'] ?? 0 ?></h3>
                <form method="POST" class="mt-2">
                    <input type="hidden" name="action" value="archiver">
                    <input type="hidden" name="type_archive" value="eleves">
                    <button type="submit" class="btn btn-primary btn-sm" <?= ($statsArchive['eleves_a_archiver'] ?? 0) == 0 ? 'disabled' : '' ?>>
    ...
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100">
            <div class="card-body text-center">
                ...
                <h3 class="fw-bold"><?= $statsArchive['lecons_a_archiver'] ?? 0 ?></h3>
                <form method="POST" class="mt-2">
                    <input type="hidden" name="action" value="archiver">
                    <input type="hidden" name="type_archive" value="lecons">
                    <button type="submit" class="btn btn-success btn-sm" <?= ($statsArchive['lecons_a_archiver'] ?? 0) == 0 ? 'disabled' : '' ?>>
                        <i class="bi bi-archive me-1"></i>Archiver
                    </button>
                </form>
            </div>
    ...
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100">
            <div class="card-body text-center">
                ...
                <h3 class="fw-bold"><?= $statsArchive['paiements_a_archiver'] ?? 0 ?></h3>
                <form method="POST" class="mt-2">
                    <input type="hidden" name="action" value="archiver">
                    <input type="hidden" name="type_archive" value="paiements">
                    <button type="submit" class="btn btn-warning btn-sm" <?= ($statsArchive['paiements_a_archiver'] ?? 0) == 0 ? 'disabled' : '' ?>>
                        <i class="bi bi-archive me-1"></i>Archiver
                    </button>
                </form>
            </div>
    ...
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100">
            <div class="card-body text-center">
                <i class="bi bi-file-earmark text-info display-4 mb-2 d-block"></i>
                <h6>Documents supprimés (+6 mois)</h6>
                <h3 class="fw-bold"><?= $statsArchive['docs_a_archiver'] ?? 0 ?></h3>
                <form method="POST" class="mt-2">
                    <input type="hidden" name="action" value="archiver">
                    <input type="hidden" name="type_archive" value="documents">
                    <button type="submit" class="btn btn-info btn-sm" <?= ($statsArchive['docs_a_archiver'] ?? 0) == 0 ? 'disabled' : '' ?>>
                        <i class="bi bi-archive me-1"></i>Archiver
                    </button>
                </form>
            </div>
