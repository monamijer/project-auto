...
if (!isset($_SESSION['2fa_pending_id'])) { header('Location: ' . BASE_URL . '/pages/login.php'); exit(); }

$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $codeEntre = trim(str_replace(' ', '', $_POST['otp_code'] ?? ''));
    $username = $_SESSION['2fa_pending_user'];
    if (time() > ($_SESSION['2fa_expire'] ?? 0)) { session_unset(); header('Location: ' . BASE_URL . '/pages/login.php?expired=1'); exit(); }
    ...
    } else { $error = 'Code incorrect ou expiré.'; }
}
...
<div class="card otp-card"><div class="card-body p-4">
    ...
    <form method="POST" id="otpForm"><div class="otp-inputs"><?php for($i=0;$i<6;$i++): ?><input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-digit" autocomplete="off"><?php endfor; ?></div><input type="hidden" name="otp_code" id="otpHidden"><button type="submit" class="btn btn-primary w-100" id="submitBtn" disabled>Vérifier</button></form>
