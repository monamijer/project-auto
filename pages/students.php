...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    if (!verify_csrf()) { $error = 'Session expirée, rechargez la page.'; }
        ...
        $msg = callProcedure('CALL sp_ajouter_eleve(?,?,?,?,?,?,@msg)', [trim($_POST['nom']),trim($_POST['prenom']),trim($_POST['nationalite']),trim($_POST['email']),trim($_POST['telephone']),(int)$_POST['formation_id']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit') {
    if (!verify_csrf()) { $error = 'Session expirée, rechargez la page.'; }
        ...
        requirePermission('crud_eleves');
        $msg = callProcedure('CALL sp_modifier_eleve(?,?,?,?,?,?,?,@msg)', [(int)$_POST['id'],trim($_POST['nom']),trim($_POST['prenom']),trim($_POST['nationalite']),trim($_POST['email']),trim($_POST['telephone']),(int)$_POST['formation_id']]);
...
if ($search !== '') {
    $students = array_filter($students, function($s) use ($search) {
        return stripos($s['nom_complet'],$search)!==false||stripos($s['email'],$search)!==false||stripos($s['telephone']??'',$search)!==false||stripos($s['nationalite']??'',$search)!==false||stripos($s['formation_nom']??'',$search)!==false;
    });
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    ...
    <button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-person-plus me-1"></i>Ajouter</button>
    <?php endif; ?>
</div>
...
<div class="card shadow-sm border-0">
    ...
    <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        <thead class="table-light"><tr><th class="ps-3">#ID</th><th>Nom complet</th><th>Nationalité</th><th>Contact</th><th>Formation</th><th>Inscription</th><th class="text-end pe-3">Actions</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td><div class="d-flex align-items-center"><div class="avatar-sm bg-primary bg-opacity-10 rounded-circle me-2 d-flex align-items-center justify-content-center" style="width:36px;height:36px;"><span class="text-primary fw-bold small"><?= strtoupper(substr($row['prenom'],0,1).substr($row['nom'],0,1)) ?></span></div><span class="fw-medium"><?= htmlspecialchars($row['nom_complet']) ?></span></div></td>
            ...
            <td><small><div><i class="bi bi-envelope text-muted me-1"></i><?= htmlspecialchars($row['email']) ?></div><?php if ($row['telephone']): ?><div><i class="bi bi-telephone text-muted me-1"></i><?= htmlspecialchars($row['telephone']) ?></div><?php endif; ?></small></td>
