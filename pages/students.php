<?php
/**
 * pages/students.php — Élèves avec tri et archivage
 */
session_start();
...
// ── ARCHIVER un élève ─────────────────────────────────────────────────────
if (isset($_GET['archive']) && hasPermission('crud_eleves')) {
    $sid = (int) $_GET['archive'];
    // Marquer comme supprimé + enregistrer dans archives
    $pdo->prepare('UPDATE utilisateurs SET deleted_at = NOW(), deleted_by = ? WHERE id = ?')->execute([$_SESSION['username'], $sid]);
    $pdo->prepare(
        "INSERT INTO archives (type_archive, nom_fichier, nb_enregistrements, periode_fin, cree_par) VALUES ('eleves', (SELECT CONCAT(prenom, ' ', nom) FROM utilisateurs WHERE id = ?), 1, CURDATE(), ?)"
    )->execute([$sid, $_SESSION['username']]);
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    <div><h1 class="h3 mb-1"><i class="bi bi-people-fill me-2 text-primary"></i>Gestion des Élèves</h1><p class="text-muted mb-0"><?= $total ?> élève(s)</p></div>
    <?php if (
        hasPermission('crud_eleves')
    ): ?><button class="btn btn-primary shadow-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-person-plus me-1"></i>Ajouter</button><?php endif; ?>
</div>
...
<?php if (!hasPermission('crud_eleves')): ?><div class="alert alert-info d-flex align-items-center"><i class="bi bi-info-circle-fill me-2"></i>Mode lecture seule.</div><?php endif; ?>
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white d-flex justify-content-between align-items-center py-3"><h5 class="mb-0"><i class="bi bi-table me-2"></i>Liste des élèves</h5><span class="badge bg-primary rounded-pill"><?= $total ?></span></div>
    <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        ...
        <tbody>
        <?php if (empty($studentsPage)): ?><tr><td colspan="7" class="text-center py-5 text-muted"><i class="bi bi-inbox display-4 d-block mb-2"></i>Aucun élève trouvé</td></tr>
        <?php else:foreach ($studentsPage as $row): ?>
        <tr>
            <td class="ps-3"><span class="badge bg-secondary bg-opacity-10 text-secondary">#<?= $row['id'] ?></span></td>
            <td><div class="d-flex align-items-center"><div class="avatar-sm bg-primary bg-opacity-10 rounded-circle me-2 d-flex align-items-center justify-content-center" style="width:36px;height:36px;"><span class="text-primary fw-bold small"><?= strtoupper(
                substr($row['prenom'], 0, 1) . substr($row['nom'], 0, 1)
            ) ?></span></div><span class="fw-medium"><?= htmlspecialchars($row['nom_complet']) ?></span></div></td>
            ...
            <td><small><div><i class="bi bi-envelope text-muted me-1"></i><?= htmlspecialchars($row['email']) ?></div><?php if (
    $row['telephone']
): ?><div><i class="bi bi-telephone text-muted me-1"></i><?= htmlspecialchars($row['telephone']) ?></div><?php endif; ?></small></td>
            <td><div class="fw-medium"><?= htmlspecialchars($row['formation_nom'] ?? '—') ?></div><small class="text-muted"><?= number_format($row['formation_prix'] ?? 0, 2) ?> $</small></td>
            ...
            <td class="text-end pe-3"><div class="btn-group btn-group-sm">
                ...
                <a href="?archive=<?= $row['id'] ?>" class="btn btn-outline-secondary" onclick="return confirm('Archiver cet élève ?')" title="Archiver"><i class="bi bi-archive"></i></a>
                <a href="?delete=<?= $row['id'] ?>" class="btn btn-outline-danger" onclick="return confirm('Déplacer vers la corbeille ?')" title="Corbeille"><i class="bi bi-trash"></i></a>
                <?php endif; ?>
            </div></td>
        </tr>
        <?php if (hasPermission('crud_eleves')): ?>
        <div class="modal fade" id="editModal-<?= $row['id'] ?>" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="POST">
            ...
            <div class="modal-body">
                <?= csrf_field() ?><input type="hidden" name="action" value="edit"><input type="hidden" name="id" value="<?= $row['id'] ?>">
                <div class="row g-3">
                    ...
                    <div class="col-md-6"><label class="form-label fw-medium">Formation <span class="text-danger">*</span></label><select name="formation_id" class="form-select" required><?php foreach (
                        $formations
                        as $f
                    ): ?><option value="<?= $f['id'] ?>" <?= $f['id'] == $row['formation_id'] ? 'selected' : '' ?>><?= htmlspecialchars($f['label']) ?></option><?php endforeach; ?></select></div>
                </div>
            </div>
...
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content"><form method="POST">
    ...
    <div class="modal-body">
        ...
        <div class="row g-3">
            <div class="col-md-6"><label class="form-label fw-medium">Prénom <span class="text-danger">*</span></label><input type="text" name="prenom" class="form-control" placeholder="Ex: Monami" required></div>
            <div class="col-md-6"><label class="form-label fw-medium">Nom <span class="text-danger">*</span></label><input type="text" name="nom" class="form-control" placeholder="Ex: Jerome" required></div>
            <div class="col-md-6"><label class="form-label fw-medium">Nationalité</label><input type="text" name="nationalite" class="form-control" placeholder="Ex: Congolaise"></div>
            <div class="col-md-6"><label class="form-label fw-medium">Email <span class="text-danger">*</span></label><input type="email" name="email" class="form-control" placeholder="monami@email.com" required></div>
