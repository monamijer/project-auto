...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-danger bg-opacity-10 py-3 d-flex justify-content-between"><h5 class="mb-0 text-danger"><i class="bi bi-cash-stack me-2"></i>Soldes impayés</h5><span class="badge bg-danger"><?= count(
        $impayes
    ) ?></span></div>
    ...
    <?php foreach ($impayes as $r): ?><tr><td class="ps-3 fw-medium"><?= htmlspecialchars($r['nom']) ?></td><td><span class="badge bg-danger"><?= number_format(
    $r['solde'],
    2
) ?> $</span></td><td><a href="<?= BASE_URL ?>/pages/student_profile.php?id=<?= $r['id'] ?>" class="btn btn-sm btn-outline-danger">Profil</a></td></tr><?php endforeach; ?>
    </tbody></table></div></div>
</div>
<?php endif; ?>
...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-warning bg-opacity-10 py-3 d-flex justify-content-between"><h5 class="mb-0 text-warning"><i class="bi bi-person-x me-2"></i>Comptes expirés</h5><span class="badge bg-warning text-dark"><?= count(
        $comptesExpires
    ) ?></span></div>
    ...
    <?php foreach ($comptesExpires as $r): ?><tr><td class="ps-3 fw-medium"><?= htmlspecialchars($r['utilisateur']) ?></td><td><?= htmlspecialchars(
    $r['date_expiration']
) ?></td><td><a href="<?= BASE_URL ?>/pages/settings.php" class="btn btn-sm btn-outline-warning">Renouveler</a></td></tr><?php endforeach; ?>
    </tbody></table></div></div>
</div>
<?php endif; ?>
...
<div class="card shadow-sm border-0 mb-4">
    <div class="card-header bg-info bg-opacity-10 py-3 d-flex justify-content-between"><h5 class="mb-0 text-info"><i class="bi bi-calendar-event me-2"></i>Leçons dans 24h</h5><span class="badge bg-info"><?= count(
        $prochainesLecons
    ) ?></span></div>
    ...
    <?php foreach ($prochainesLecons as $r): ?><tr><td class="ps-3 fw-medium"><?= date('d/m H:i', strtotime($r['date_lecon'])) ?></td><td><?= htmlspecialchars(
    $r['student_nom']
) ?></td><td><?= htmlspecialchars($r['instructor_nom']) ?></td></tr><?php endforeach; ?>
    </tbody></table></div></div>
</div>
<?php endif; ?>
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-secondary bg-opacity-10 py-3 d-flex justify-content-between"><h5 class="mb-0"><i class="bi bi-car-front me-2"></i>Véhicules indisponibles</h5><span class="badge bg-secondary"><?= count(
        $vehiculesIndispos
    ) ?></span></div>
    ...
    <?php foreach ($vehiculesIndispos as $r): ?><tr><td class="ps-3 fw-medium"><?= htmlspecialchars($r['designation']) ?></td><td><?= htmlspecialchars(
    $r['immatriculation']
