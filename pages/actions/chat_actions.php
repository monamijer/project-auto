...
try {
    if ($action === 'create_conversation') {
        ...
        if ($msgType === 'text' && empty($c)) {
            echo json_encode(['s' => 0, 'e' => 'Message vide.']);
            exit();
        }
        ...
        foreach ($msgs as &$msg) {
            $msg['photo_url'] = getPhotoUrl($msg['sender_id']);
        }
        unset($msg);
        foreach ($parts as &$p) {
            $p['photo_url'] = getPhotoUrl($p['utilisateur_id']);
        }
        unset($p);
        ...
        echo json_encode(['s' => 0, 'e' => 'Action inconnue']);
    }
} catch (Exception $e) {
    echo json_encode(['s' => 0, 'e' => $e->getMessage()]);
}
