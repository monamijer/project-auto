...
<div class="d-flex justify-content-between align-items-center mb-4 no-print">
    ...
    <div>
        <button onclick="window.print()" class="btn btn-primary btn-sm"><i class="bi bi-printer me-1"></i>Imprimer</button>
        <a href="<?= BASE_URL ?>/pages/payments.php" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i>Retour</a>
    </div>
</div>
...
<div class="card shadow-sm border-0">
    <div class="card-body p-4">
        <div class="row mb-4">
            <div class="col-6">
                <h3 class="fw-bold mb-1"><?= htmlspecialchars(getConfig('nom_ecole') ?: 'Auto École Pro') ?></h3>
                <p class="text-muted mb-0 small"><?= htmlspecialchars(getConfig('adresse') ?: '') ?></p>
                <p class="text-muted mb-0 small"><?= htmlspecialchars(getConfig('telephone') ?: '') ?></p>
                <p class="text-muted mb-0 small"><?= htmlspecialchars(getConfig('email_ecole') ?: '') ?></p>
            </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-6 text-end">
                ...
                <p class="text-muted small mb-0">Prix total : <?= number_format($recu['formation_prix'], 2) ?> <?= htmlspecialchars(getConfig('devise') ?: '$') ?></p>
            </div>
        </div>
        ...
        <hr>
        <div class="row mt-4">
            <div class="col-6"><p class="text-muted small mb-1">Signature de l'élève</p><div style="border-bottom:1px solid #ccc;width:150px;margin-top:40px;"></div></div>
            <div class="col-6 text-end"><p class="text-muted small mb-1">Cachet de l'école</p><div style="border-bottom:1px solid #ccc;width:150px;margin-top:40px;margin-left:auto;"></div></div>
        </div>
        <p class="text-muted small text-center mt-4 mb-0">Document généré le <?= date('d/m/Y à H:i') ?></p>
    </div>
</div>
...
@media print {
    ...
    .sidebar,.mobile-topbar,.sidebar-backdrop,.no-print,.main-content{margin-left:0!important;padding:0!important;}
