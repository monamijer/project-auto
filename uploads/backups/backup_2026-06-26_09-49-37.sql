...
INSERT INTO `documents` VALUES (1,19,'Photo identité','Monami.png','19/doc_6a3b7070b764e.png',1915,1,'Admin','2026-06-24 07:51:44',NULL),(2,6,'Photo identité','Monami.png','6/doc_6a3cc538bc757.png',1915,1,'admin','2026-06-25 08:05:44','2026-06-26 08:32:25');
...
INSERT INTO `password_resets` VALUES (1,6,'74a32d20db87dd5635939b10b879bb8bca4d7bc2563ccd6024ee3a907a396fd5','2026-06-25 11:20:14',1,'2026-06-25 08:20:14');
...
INSERT INTO `sauvegarde_auto` VALUES (1,'2026-05-07 12:00:00',51,26),(2,'2026-05-12 12:00:00',51,26),(3,'2026-05-15 12:00:00',50,26),(4,'2026-05-24 12:00:00',50,26),(5,'2026-05-26 12:00:00',50,26),(6,'2026-05-28 12:00:00',50,26),(7,'2026-05-30 12:00:00',50,26),(8,'2026-05-31 12:00:00',50,26),(9,'2026-06-01 13:11:46',50,26),(10,'2026-06-02 12:40:27',50,26),(11,'2026-06-03 12:34:12',50,26),(12,'2026-06-07 12:00:00',50,26),(13,'2026-06-08 12:00:00',50,26),(14,'2026-06-11 12:00:00',50,26),(15,'2026-06-14 19:03:06',50,26),(16,'2026-06-15 12:00:00',50,26),(17,'2026-06-17 12:00:00',51,26),(18,'2026-06-18 12:00:00',51,26),(19,'2026-06-19 12:25:29',51,26),(20,'2026-06-21 18:20:46',51,29),(21,'2026-06-24 12:00:00',51,29),(22,'2026-06-25 12:00:00',51,29);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_derniers_paiements` AS select `p`.`id` AS `id`,`p`.`date_paiement` AS `date_paiement`,`p`.`montant` AS `montant`,`p`.`methode` AS `methode`,concat(`u`.`prenom`,' ',`u`.`nom`) AS `student_nom` from (`paiement` `p` join `utilisateurs` `u` on(`p`.`utilisateur_id` = `u`.`id` and `u`.`deleted_at` is null)) order by `p`.`date_paiement` desc limit 5 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_prochaines_lecons` AS select `l`.`id` AS `id`,`l`.`utilisateur_id` AS `utilisateur_id`,`l`.`instructeur_id` AS `instructeur_id`,`l`.`vehicule_id` AS `vehicule_id`,`l`.`date_lecon` AS `date_lecon`,`l`.`statut` AS `statut`,concat(`u`.`prenom`,' ',`u`.`nom`) AS `student_nom`,concat(`i`.`prenom`,' ',`i`.`nom`) AS `instructor_nom`,concat(`v`.`marque`,' ',`v`.`modele`) AS `vehicle_nom` from (((`lecons` `l` join `utilisateurs` `u` on(`u`.`id` = `l`.`utilisateur_id`)) join `instructeurs` `i` on(`i`.`id` = `l`.`instructeur_id`)) join `vehicules` `v` on(`v`.`id` = `l`.`vehicule_id`)) where `l`.`statut` = 'programmée' and `l`.`date_lecon` >= current_timestamp() order by `l`.`date_lecon` limit 5 */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `v_stats_examens` AS select `f`.`nom` AS `formation_nom`,count(distinct `u`.`id`) AS `total_eleves`,count(distinct `e`.`id`) AS `eligibles` from ((`formations` `f` left join `utilisateurs` `u` on(`u`.`formation_id` = `f`.`id` and `u`.`deleted_at` is null)) left join `v_examens_eligibles` `e` on(`e`.`id` = `u`.`id`)) group by `f`.`id`,`f`.`nom` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-06-26  9:49:38
