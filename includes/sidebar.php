...
$unreadMessages = 0;
try {
    $stmt = $pdo->prepare('SELECT non_lus FROM v_chat_unread WHERE utilisateur_id = ?');
...
<nav class="sidebar" id="appSidebar">
    ...
    <div class="sidebar-header">
        ...
        <div class="mt-2 d-flex align-items-center gap-2">
            <a href="<?= BASE_URL ?>/pages/profile.php" style="text-decoration:none;flex-shrink:0;">
                <?php if ($photoUrl): ?>
                <img src="<?= htmlspecialchars($photoUrl) ?>" alt="" style="width:34px;height:34px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,255,255,.3);">
                <?php else: ?>
                <div style="width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.15);display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:.9rem;border:2px solid rgba(255,255,255,.3);">
                    <?= strtoupper(substr($_SESSION['username'] ?? 'U', 0, 1)) ?>
                </div>
                <?php endif; ?>
            </a>
            <div style="flex:1;min-width:0;">
                <div class="text-white small fw-medium text-truncate" style="line-height:1.2;"><?= htmlspecialchars($_SESSION['username'] ?? '') ?></div>
                <span class="badge <?= getRoleBadgeClass() ?>" style="font-size:.6rem;"><?= getRoleLabel() ?></span>
            </div>
            <button class="theme-toggle" onclick="toggleTheme()" title="Mode sombre/clair" type="button" style="flex-shrink:0;position:relative;z-index:1;"></button>
        </div>
    </div>
    ...
    <div class="sidebar-body">
        ...
        <ul class="nav flex-column px-2">
            <?= navLink('rapports.php', 'bi-bar-chart', 'Rapports', $currentFile) ?>
        ...
        <ul class="nav flex-column px-2">
            ...
            <?= navLink('aide.php', 'bi-book', 'Guide', $currentFile) ?>
            <?= navLink('uml.php', 'bi-diagram-3', 'UML', $currentFile) ?>
        </ul>

    </div>

    <div class="sidebar-footer">
        <a href="<?= BASE_URL ?>/pages/actions/logout.php" class="btn btn-sm btn-outline-light w-100"><i class="bi bi-box-arrow-right me-1"></i>Déconnexion</a>
    </div>
</nav>